...
const EditMyProfile: React.FC<{ selectedUser?: any }> = ({ selectedUser }) => {
  ...
  const handleSaveButtonClick = () => {
    ...
    let cloneExistingUser = [...storedUserData];
    let foundExistingUserIndex = cloneExistingUser.findIndex((userObj: any) => {
      userObj.userid = editedUserData.userid;
    });

    //found existing user
    if (foundExistingUserIndex > 0) {
      // perform update action
      let selectedExistUser = cloneExistingUser[foundExistingUserIndex];
      ...
      storedUserData[foundExistingUserIndex] = selectedExistUser;

      // update local storage 's userList
      if ((foundExistingUserIndex = 1)) {
      ...
      // localStorage.setItem("userList", JSON.stringify(editedUserData));
    }
